...
import AI_Writer.src.utils
from AI_Writer.src.model import GPT, GPTConfig
import AI_Writer.config as config
...
class Writer:
    def __init__(self):
        ...
        model_name = "/path/to/writer/AI_Writer/model/{}".format(config.DATA_NAME)  # 预训练好的模型
        word_name = "/path/to/writer/AI_Writer/model/{}".format(config.DATA_NAME)
        ...
        if config.RUN_DEVICE == "dml":
            ...
            if config.RUN_DEVICE == "cpu":
                ...
                m2 = torch.load(model_name).state_dict()
        ...
        for i in range(int(length_of_each)):
            ...
            with torch.no_grad():
                if config.RUN_DEVICE == "dml":
                    ...
                    xxx = torch.tensor(x[-config.CTX_LEN :], dtype=torch.long)[None, ...]
                    if config.RUN_DEVICE == "gpu":
                        xxx = xxx.cuda()
            ...
            if self.itos[int(x[real_len - 1])] == "\n":
                char = AI_Writer.src.utils.sample_logits(
